...
def safe_batch_decode(batch, tokenizer):
...
def compute_metrics(eval_preds):
    ...
    decoded_labels = safe_batch_decode(labels, tokenizer)
...
def truncate_input(text, tokenizer, max_tokens=1978):
...
def predict(test_df, model, tokenizer, batch_size=4, max_input_tokens=1978, max_new_tokens=70):
    ...
    test_df["text"] = test_df["text"].apply(lambda x: truncate_input(x, tokenizer, max_input_tokens))
